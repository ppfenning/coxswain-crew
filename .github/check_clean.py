...
The whole claim of this repo is that a seat outlives the workplace it was
...
for path in sorted(pathlib.Path(".").rglob("*")):
    ...
    for number, line in enumerate(path.read_text(encoding="utf-8", errors="replace").splitlines(), 1):
        ...
        if hit:
            problems.append(f"{path}:{number}: names '{hit.group(0)}' — a seat must not assume a vendor")
